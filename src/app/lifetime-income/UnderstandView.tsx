    ...
    <div>
      ...
      <div className="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-2xl p-5 mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">Career start year</label>
            <input
              type="number"
              value={startYear}
          ...
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">Starting monthly income</label>
          ...
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">Current monthly income</label>
          ...
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">Roughly, % you save</label>
            <div className="flex items-center border border-[var(--border-default)] rounded-lg px-3 focus-within:border-[var(--green)]">
              <input
                value={saveRatePct}
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData}>
                ...
                <Bar dataKey="cumulativeIncome" name="Total earned (cumulative)" fill="var(--blue)" />
                <Line type="monotone" dataKey="cumulativeSaved" name="What actually stayed (savings)" stroke="var(--red)" strokeWidth={2.5} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-2xl p-6 mb-4">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="keptShare" name="Kept" stackId="a" stroke="var(--blue)" fill="var(--blue-soft-bg)" />
                <Area type="monotone" dataKey="spentShare" name="Spent" stackId="a" stroke="var(--red)" fill="var(--red-soft-bg)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="flex gap-3 items-start bg-[var(--gold-bg)] border border-[var(--gold)] rounded-xl p-4 mt-4">
            ...
            <div className="text-xs text-[var(--gold-text-body)] leading-relaxed">
              <strong className="text-[var(--gold-text-strong)]">How to read this.</strong> Each point shows what
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-6">
        <div className="bg-[var(--surface-card)] border border-[var(--blue)] rounded-2xl p-5">
          <div className="text-[11px] tracking-[0.08em] uppercase text-[var(--muted)] mb-2">You&apos;ve earned</div>
          <div className="font-serif text-2xl font-semibold text-[var(--blue)] mb-1">SAR {fmtCompact(earned)}</div>
          <div className="text-xs text-[var(--ink-2)]">across your working life so far</div>
        </div>
        <div className="bg-[var(--surface-card)] border border-[var(--green)] rounded-2xl p-5">
          <div className="text-[11px] tracking-[0.08em] uppercase text-[var(--muted)] mb-2">You kept</div>
          <div className="font-serif text-2xl font-semibold text-[var(--green-dark)] mb-1">SAR {fmtCompact(kept)}</div>
          <div className="text-xs text-[var(--ink-2)]">that&apos;s {keptPct.toFixed(0)}% of everything you earned</div>
        </div>
        <div className="bg-[var(--surface-card)] border border-[var(--red)] rounded-2xl p-5">
          <div className="text-[11px] tracking-[0.08em] uppercase text-[var(--muted)] mb-2">You spent</div>
          <div className="font-serif text-2xl font-semibold text-[var(--red)] mb-1">SAR {fmtCompact(spent)}</div>
          <div className="text-xs text-[var(--ink-2)]">{(100 - keptPct).toFixed(0)}% flowed through and is gone</div>
        </div>
      </div>
      ...
      <div className="bg-[var(--surface-card)] border-[1.5px] border-[var(--green)] rounded-2xl p-6 mb-6">
        ...
        <div className="font-serif text-lg text-[var(--ink)] leading-relaxed mb-2">
          Over your working life, about <strong className="text-[var(--green-dark)] font-semibold">SAR {fmt(earned)}</strong> has
          passed through your hands. You kept <strong className="text-[var(--green-dark)] font-semibold">SAR {fmt(kept)}</strong>.
        </div>
          ...
          <div className="pt-4 border-t border-[var(--border-default)] text-sm text-[var(--ink-2)] leading-relaxed">
            {reflection === 'happy' && (
              <>
                ...
                <strong className="text-[var(--ink)] font-medium">work harder</strong> — invested rather
            ...
            {reflection === 'unsure' && (
              <>
                ...
                <strong className="text-[var(--ink)] font-medium">bought you</strong> — progress, joy, or
            ...
            {reflection === 'unhappy' && (
              <>
                ...
                bought <strong className="text-[var(--ink)] font-medium">neither progress nor enjoyment</strong>{' '}
      ...
      <div className="flex gap-3 items-start bg-[var(--gold-bg)] border border-[var(--gold)] rounded-xl p-4">
        ...
        <div className="text-xs text-[var(--gold-text-body)] leading-relaxed">
          <strong className="text-[var(--gold-text-strong)]">MalMind keeps this in view.</strong> This lifetime picture
